...
resource "aws_elasticsearch_domain" "es_domain" {
  ...
  depends_on = [
    aws_iam_service_linked_role.es, aws_cloudwatch_log_group.es_cloudwatch_log_group
  ]
  ...
  # log_publishing_options-( Made Changes )
dynamic "log_publishing_options" {
    for_each = toset(var.cloudwatch_log_group)
     content {
      log_type                 = log_publishing_options.key
      cloudwatch_log_group_arn = format("arn:aws:logs:%s:%s:log-group:%s-%s:*", var.region, data.aws_caller_identity.current.account_id, var.domain_name,lower(log_publishing_options.key) )
      enabled                  = "true"
    }
  }
